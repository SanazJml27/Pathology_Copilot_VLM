...
A local research prototype for interactive pathology image analysis using a browser-based ROI selection interface and a local PA-LLaVA pathology vision-language model backend.

This MVP allows a user to upload an H&E pathology image, select a region of interest, ask questions about the selected ROI, receive PA-LLaVA-generated visual findings, optionally upload synthetic EHR/context text, and generate a downloadable draft pathology report.
...
Add one screenshot of the running UI here.

Save your screenshot as:

```text
docs/screenshots/ui-screenshot.png

Demo Goal

The goal of this project is to demonstrate a local pathology co-pilot workflow:
...
Model returns ROI-based pathology description
    ↓
Optional report generation creates downloadable HTML draft

The system is designed around ROI-based visual analysis, not full-slide diagnosis.
...
Chat behavior
Send with button click or Enter.
Ask multiple follow-up questions about the same selected ROI.
...
EHR is treated as contextual metadata.
PA-LLaVA image interpretation is kept primarily image-based to avoid clinical text biasing the visual description.
Report generation
Generates a draft report from the selected ROI.
Displays the report in the right-side panel.
Allows downloading the report as an HTML file.
Includes limitations and research-only disclaimer.
System Architecture
...
These are excluded because they are large and may have separate licenses or access requirements.
...
cd ~/Documents/pathology-copilot-mvp
...
If requirements.txt is incomplete, install the core packages manually:
...
The PA-LLaVA environment should include:
...
The exact PA-LLaVA/XTuner environment may require version pinning depending on the PA-LLaVA release.
...
~/Documents/pathology-copilot-mvp/external/PA-LLaVA-clean
...
Create a .env file locally if desired:
...
PALAVA_ROOT=/Users/YOUR_USERNAME/Documents/pathology-copilot-mvp/external/PA-LLaVA-clean
...
export PALAVA_ROOT="/Users/YOUR_USERNAME/Documents/pathology-copilot-mvp/external/PA-LLaVA-clean"
export PALAVA_LLAVA_PATH="weights/domain_alignment_weight_ft"
...
Start the PA-LLaVA service on port 9000.
Start the main UI on port 8000.
Open the browser UI.
Upload an H&E pathology image.
Click Select Region.
Draw an ROI over the image.
Confirm that the selected ROI preview appears in the chat.
...
Ask a follow-up question without selecting a new ROI:
What cell morphology and tissue architecture are visible in this selected region?
Click Generate Report.
Review the generated report in the right-side panel.
...
It helps demonstrate targeted pathology questioning.
...
Troubleshooting
UI opens but model does not answer

Check that PA-LLaVA service is running:
...
Follow-up question fails after first answer
...
Also use a question that asks for a descriptive pathology paragraph.
